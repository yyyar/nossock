...
module.exports = {
     ...
     * Create client
     */
    createClient: function(opts, callback) {
        opts.type = opts.type || 'tcp';
        if (['tls', 'tcp'].indexOf(opts.type) == -1) {
            throw new Error('Type could be tls or tcp');
        }
        var socket = (opts.type == 'tls' ? tls : net).connect(opts);
     ...
     * Create server
     */
    createServer: function(opts, callback) {
        opts.type = opts.type || 'tcp';
        if (['tls', 'tcp'].indexOf(opts.type) == -1) {
            throw new Error('Type could be tls or tcp');
        }
        return (opts.type == 'tls' ? tls : net).createServer(opts, function(socket) {
            callback( new Nossock(socket) );
        });
    }
};
