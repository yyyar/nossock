...
var Nossock = function(socket) {
    ...
    ['connect', 'end', 'timeout', 'error', 'close', 'timeout'].forEach(function (e) {
            ...
            if ((match = pattern.exec(msg.name)) !== null) {
                ...
                if (gotName === name) {
                    callback(msg.body, function(err, respBody) {
                        ...
                        if (respBody instanceof Buffer) {
                            self.send('resp:' + name + ':' + id, Buffer.concat( [new Buffer(err + '\0'), respBody ]));
                        } else self.send('resp:' + name + ':' + id, [err, respBody]);
                    });
                }
            }
    ...
    this.sendReq = function(name, body, callback) {
        ...
        self.once(respName, function (resp) {
            ...
            if (resp instanceof Buffer) {
                var errEndIndex = resp.indexOf(0);

                var err = resp.slice(0, errEndIndex).toString(),
                    data = resp.slice(errEndIndex + 1);

                callback(err, data);

            } else callback(resp[0], resp[1]);
        });

        var timeout = setTimeout(function() {
            self.removeAllListeners(respName);
            if (!isGotResponse) {
                callback(new Error('Timeout ' + self.config.reqTimeout +'ms' + name + ' ' + JSON.stringify(body)));
            }
