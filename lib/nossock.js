...
var Nossock = function(socket) {
    ...
    var parser = new Parser()
            ...
            .add('%bodyLen', 'body', function(msg, b) {
                return msg.type === 'j' ? JSON.parse(b.toString()) : Buffer.from(b);
            });
    ...
    this.send = function(name, body, callback) {
        ...
        body = isJson ? Buffer.from(JSON.stringify(body)) : body;
        ...
        // type ::= Buffer(b | j)
        socket.write(Buffer.from(isJson ? 'j' : 'b'));

        // nameLen ::= Buffer(1)
        socket.write(Buffer.from([name.length]));

        // name ::= Buffer(nameLen)
        socket.write(Buffer.from(name));
